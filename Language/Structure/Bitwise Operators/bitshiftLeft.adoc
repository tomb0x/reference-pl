...
Poniższy przykład może być użyty do wydrukowania wartości odebranego bajtu na monitorze szeregowym, przy użyciu operatora przesunięcia w lewo. Aby wydrukować dwójkową reprezentację bajtu, analiza rozpoczyna się od najmniej znaczącego bitu (LSB) do najbardziej znaczącego bitu (MSB):
